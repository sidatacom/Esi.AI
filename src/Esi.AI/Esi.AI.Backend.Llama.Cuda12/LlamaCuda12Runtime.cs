...
/// <summary>Loads GGUF models and serves normalized chat requests through LLamaSharp's CUDA 12 runtime.</summary>
public sealed class LlamaCuda12Runtime : IBackendRuntime
{
    private const string VariantId = "llama.cuda12";
    ...
    /// <summary>Initializes a CUDA 12 runtime using the application runtime directory.</summary>
    /// <param name="applicationDirectory">An optional directory containing the packaged CUDA runtime.</param>
    public LlamaCuda12Runtime(string? applicationDirectory = null)
    {
    ...
    public async Task LoadAsync(BackendLoadRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var configuration = request.Configuration.Deserialize<LoadModelRequest>(ConfigurationJsonOptions)
            ?? throw new ArgumentException("A CUDA 12 model configuration is required.", nameof(request));
        ...
        try
        {
            ...
            var modelParams = new ModelParams(Path.GetFullPath(request.ModelPath))
            {
                ...
                Devices = ResolveCudaDevices(configuration.Devices)
            };
    ...
    private static IReadOnlyList<string> ResolveCudaDevices(IReadOnlyList<string>? requestedDevices)
    {
        ...
        if (unavailable.Length > 0)
            throw new ArgumentException($"CUDA device(s) were not found: {string.Join(", ", unavailable)}.", nameof(requestedDevices));

        return requestedDevices;
    }

    private IReadOnlyList<DeviceStatus> GetCudaDevices()
    {
        try
        {
...
internal static class Cuda12RuntimeFiles
{
    internal static string GetRuntimeDirectory(string applicationDirectory)
    {
        if (!OperatingSystem.IsLinux() || !string.Equals(RuntimeInformation.ProcessArchitecture.ToString(), "X64", StringComparison.Ordinal))
            throw new PlatformNotSupportedException("The LLama CUDA 12 backend package currently supports Linux x64 only.");

        return Path.Combine(applicationDirectory, "runtimes", "linux-x64", "native", "cuda12");
    }
    ...
    {
        var requiredFiles = new[] { "libllama.so", "libggml.so", "libggml-base.so", "libggml-cuda.so", "libmtmd.so" };
        var missing = requiredFiles.Where(file => !File.Exists(Path.Combine(runtimeDirectory, file))).ToArray();
        if (missing.Length > 0)
            throw new InvalidOperationException($"The LLama CUDA 12 runtime is incomplete. Missing: {string.Join(", ", missing)} in '{runtimeDirectory}'.");
    }
}