...
- Lies nach Hot Reload, Restart oder einem fehlgeschlagenen Browsercheck mit `csdevkit.debug.output.diagnostics` die gepufferte Debug-Console aus. Prüfe zuerst `lastLine` auf den neuesten Fehler oder Readiness-Status und bei Bedarf `output` für den Kontext. Bei `null` zuerst mit `csdevkit.debug.active.session` prüfen, ob überhaupt eine Debugsession läuft.
...
- Nach einer Änderung an `.razor`, `.razor.css`, CSS oder Markup muss bei einer aktiven Studio-Debugsession als unmittelbar nächste Aktion `csdevkit.debug.hotReload` ausgeführt werden. Ein separater Build, Test, `get_errors` oder eine reine Diff-Prüfung ist davor keine zulässige Validierung.
